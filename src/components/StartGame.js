...
const StartGame = ({}) => {
    ...
    function chosenCharacter(x) {
        dispatch(setPlayer(x));
        navigate("/main");
    }

    return (
        <div className='startGameBox d-flex column align-center'>
            <h1>Choose the character you want to play with:</h1>
            <div className='d-flex wrap mt-40 justify-center'>
                {characters.map((x, i) => <div className='characterBox' onClick={() => chosenCharacter(x)} key={i}>
                    <h2 className='text-center'>{x.race}</h2>
                    <img src={x.image} alt="" />
