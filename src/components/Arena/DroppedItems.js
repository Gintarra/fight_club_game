...
const DroppedItems = () => {
    ...
    const itemsAmount = Math.floor(Math.random() * stateMonster.maxItemsDrop) + 1;
    const itemsState = useSelector((state) => state.items.value);
    const dispatch = useDispatch();

    let itemsArray2 = [...new Array(itemsAmount)];
    let itemsArray = [];
    const [itemsArr, setItemsArr] = useState([]);



    itemsArray2.map(x => itemsArray.push(dropItems[randomNumber(dropItems.length - 1)]));


    useEffect(() => {
        setItemsArr([...itemsArray]);
    }, [])
    ...
    function addToInventory(x, i) {
        if (state.inventorySlots > itemsState.length) {
            ...
            const filteredArray = helpArray.filter((x, index) => i !== index)
            setItemsArr(filteredArray);
        } else {
            alert("You don't have empty slot")
        }
    }
    ...
    return (
        <div className='dropBox2'>
                    ...
                    runCallback(() => {
                        ...
                        for (let i = 0; i < state.inventorySlots; i++) {
                            row.push(<div key={i} className='smallBox2'>
                                {!!itemsState[i] && <div> <img src={itemsState[i].image} alt="" />
                                </div>}
                            </div>);
                        }
                        return row;
            ...
            {itemsArr.length > 0 ? itemsArr.map((x, i) =>
                <div key={i} className='dropBox'>
                    <img className='dropImg' src={x.image} alt="" />
                    <h3>{x.price}</h3>
                    <button onClick={() => addToInventory(x, i)}>Add to inventory</button>
                </div>
            ) : "No items"}
