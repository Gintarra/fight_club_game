...
import { minusGold } from '../features/player';
...
const Shop = () => {
    ...
    const [getLink, setLink] = useState("w");
...
//prideda prie weapons effect tikrus, vietoj pavadinimu
    trader.weapons.map(y => {
           const newVal2 =[]
        y.effects.map(z => {
              newVal2.push(effects[z]);
        })
         y.effects = [...newVal2]
    })
    ...
    function buyEquickment(item) {
        if (itemsState.length < state.inventorySlots && state.gold >= item.price) {
            dispatch(setItems(item));
            dispatch(minusGold(item.price));
        } else {
            alert("nope")
        }
    ...
    return (
        <div>
            ...
            <div className='d-flex'>
                <div className='grow1 shopBox'>
                    <ToolbarMini data={getData} />
                    <div className='d-flex wrap m-5 justify-center'>
                        {getLink === "w" && trader.weapons.map((x, i) => <div onClick={() => buyEquickment(x)} className='m-5 weaponBox d-flex column' key={i}>
                            <img src={x.image} alt="" />
                            <h5>Max damage: {x.maxDamage}</h5>
                            <h5>Price: {x.price}</h5>
                            <h5>Energy per hint: {x.energyPerHit}</h5>
                            <h5><div className='d-flex wrap'>Effects: {x.effects.length > 0 ? x.effects.map((y, i) => <div key={i}>{y.title}</div>) : "none"} </div></h5>
                        </div>)}
                        {getLink === "p" && trader.potions.map((x, index) => <div onClick={() => buyEquickment(x)} className='m-5 weaponBox d-flex column' key={index}>
                            <img src={x.image} alt="" />
                            <h5> {x.title}</h5>
                            <h5>Price: {x.price}</h5>
                            <h4>Effect: {x.title.includes("Energy") ? x.effect.energy : x.effect.health}</h4>
                        </div>)}
                    </div>
